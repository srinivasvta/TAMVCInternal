    ...
    public class BLLClassified
    {
        ...
        public static bool AddClassified(ClassifiedViewModel addClassified)
        {
            if (addClassified != null)
            {
                ...
                using (TAC_Team5Entities entities = new TAC_Team5Entities())
                {
                    newClassfied.CategoryId = (from item in entities.TAC_Category
                                               where item.CategoryName.Equals(addClassified.CategoryName)
                                               select item.CategoryId).FirstOrDefault();
                    newClassfied.CreatedBy = (from user in entities.TAC_User
                                              where user.Email.Equals(addClassified.Createdby)
